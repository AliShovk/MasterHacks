...
if (mb_strlen($q) >= 2) {
    ...
    try {
        ...
        // MATCH...AGAINST with BOOLEAN MODE, append * for prefix matching
        $searchTerm = $q . '*';
        ...
        // Fallback to LIKE if FULLTEXT is not available
        try {
            $stmt = $pdo->prepare(
...
<meta name="description" content="Результаты поиска по запросу «<?= h($q) ?>» на MasterHacks. Полезные видео для дома, хозяйства и строительства.">
    ...
    <?php elseif ($searched): ?>
        <?php if (count($results) > 0): ?>
            <div class="results-info">
                Найдено: <?= count($results) ?> результат<?= count($results) === 1 ? '' : (count($results) >= 5 ? 'ов' : 'а') ?>
                по запросу «<?= h($q) ?>»
            </div>
        ...
        <?php else: ?>
            <div class="no-results">
                ...
                <p style="font-size:13px;margin-top:8px;">Попробуйте изменить запрос</p>
            </div>
        <?php endif; ?>
    <?php else: ?>
        <div class="no-results">
            <i class="fas fa-search"></i>
            <p>Введите запрос для поиска видео</p>
        </div>
    <?php endif; ?>
</div>
