<?php
/**
 * MasterHacks - Category/Tag page
 * Filters videos by JSON tag
 */
    ...
    try {
        ...
        $sql = "SELECT v.id, v.filename, v.file_type, v.description, v.likes, v.views,
                ...
                WHERE v.status = 'approved'
                  AND JSON_CONTAINS(v.tags, JSON_QUOTE(:tag))
        ...
        // Fallback: try LIKE on tags column
        try {
            $stmt = $pdo->prepare(
...
<title>Категория: <?= h($tag) ?> | MasterHacks</title>
<meta name="description" content="Видео в категории «<?= h($tag) ?>» на MasterHacks. Полезные видео для дома, хозяйства и строительства.">
...
.header {
    ...
    border-bottom: 1px solid rgba(252,123,7,0.15);
    padding: 10px 16px;
...
.back-link:hover { background: rgba(252,123,7,0.2); text-decoration: none; }
.search-form { flex: 1; display: flex; gap: 8px; min-width: 0; }
.search-input {
    flex: 1; min-width: 0;
    background: rgba(255,255,255,0.08);
    border: 1px solid rgba(252,123,7,0.25);
...
.search-btn {
    ...
    background: rgba(252,123,7,0.85);
...
.breadcrumb {
    ...
    font-size: 13px; margin-bottom: 16px; color: rgba(255,255,255,0.5);
}
.breadcrumb a { color: rgba(255,255,255,0.5); }
.breadcrumb a:hover { color: #fc7b07; }
.breadcrumb .sep { color: rgba(255,255,255,0.25); }
.breadcrumb .current { color: #fc7b07; font-weight: 600; }
...
.no-results i { font-size: 48px; color: rgba(252,123,7,0.3); margin-bottom: 16px; display: block; }
...
.result-item:hover {
    border-color: rgba(252,123,7,0.25);
...
.result-author {
    font-size: 12px; color: #fc7b07; margin-bottom: 6px;
}
.result-meta {
...
@media (max-width: 480px) {
    ...
    .logo { height: 28px; }
    .result-thumb { width: 80px; height: 105px; }
...
<div class="container">
    ...
    <div class="breadcrumb">
        <a href="/">Главная</a>
        <span class="sep">›</span>
        <span class="current">Категория: <?= h($tag) ?></span>
    </div>
    ...
    <?php elseif ($searched): ?>
        <?php if (count($results) > 0): ?>
            <div class="results-info">
                Найдено: <?= count($results) ?> видео в категории «<?= h($tag) ?>»
            </div>
                    ...
                    <div class="result-thumb">
                        <?php if ($isVideo): ?>
                            <video src="<?= $mediaSrc ?>" muted preload="metadata"
                                   onerror="this.parentElement.innerHTML='<i class=\\'fas fa-video\\' style=\\'color:rgba(252,123,7,0.4);font-size:24px\\'></i>'"></video>
                            <div class="play-icon"><i class="fas fa-play"></i></div>
                        <?php else: ?>
                            <img src="<?= $mediaSrc ?>" alt="" loading="lazy"
                                 onerror="this.parentElement.innerHTML='<i class=\\'fas fa-image\\' style=\\'color:rgba(252,123,7,0.4);font-size:24px\\'></i>'">
                        <?php endif; ?>
                    </div>
        ...
        <?php else: ?>
            <div class="no-results">
                ...
                <p style="font-size:13px;margin-top:8px;">В категории «<?= h($tag) ?>» пока нет видео</p>
            </div>
        <?php endif; ?>
    <?php else: ?>
        <div class="no-results">
